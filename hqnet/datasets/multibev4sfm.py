...
class MultiBEV4SFM(BaseDataset):
    ...
    def load_annotations(self):
        self.logger.info('Loading MultiBEV4SFM annotations...')
        self.data_infos = []
        max_sample_idx = len(self._annotations) - 1 - (self.cfg.queue_length - 1) * self.cfg.overlap_scale
        for anno_idx in self._annotations:
            img_path = []
            label = []
            track = []
            old_anno = []
            if anno_idx > max_sample_idx:
                continue
            for qi in range(self.cfg.queue_length):
                anno_file = self._annotations[anno_idx+qi*self.cfg.overlap_scale]
                img_path.append({'img_path_{}'.format(qi):
                        anno_file['old_anno']['path']})
                label.append({'label_{}'.format(qi):
                anno_file['label']})
                track.append({'track_{}'.format(qi):
                anno_file['track']})
                old_anno.append({'old_anno_{}'.format(qi):
                anno_file['old_anno']})
    ...
    def __getitem__(self, idx):
        ...
        samples = []
        for qi in range(self.cfg.queue_length):
            data_info = self.get_data_info(data_infos, qi)
    ...
    def get_data_info(self, data_infos, qi):
        data_info = {
            'img_path': data_infos['img_path'][qi]['img_path_{}'.format(qi)],
            'label': data_infos['label'][qi]['label_{}'.format(qi)],
            'track': data_infos['track'][qi]['track_{}'.format(qi)],
            'old_anno': data_infos['old_anno'][qi]['old_anno_{}'.format(qi)],
        }
        return data_info