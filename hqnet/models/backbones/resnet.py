...
from hqnet.models.registry import BACKBONES
...
@BACKBONES.register_module
class ResNetWrapper_Reduced(nn.Module):
    def __init__(self,
                 resnet='resnet18',
                 pretrained=True,
                 replace_stride_with_dilation=[False, False, False],
                 ...
                 in_channels = [16, 32, 64, 128],
                 cfg=None):
        super(ResNetWrapper_Reduced, self).__init__()
        self.cfg = cfg
        self.in_channels = in_channels


        self.model = resnet18_reduced(pretrained=pretrained)
        self.out = None
        if out_conv:
            out_channel = 51
            for chan in reversed(self.in_channels):
...
def resnet18_reduced(pretrained=False, progress=True, expansion=1, **kwargs):
    ...
    model = ResNet_Reduced(block=BasicBlock, layers=[1, 2, 2, 2], in_channels=channels, **kwargs)
